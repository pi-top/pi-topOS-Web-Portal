...
from tests.data.keyboard_data import keyboard_code_list, keyboard_variants_list, keyboard_query_data, keyboard_current
...
def test_current_keyboard(app, mocker):
    ...
    run_mock.assert_called_once_with(['setxkbmap', '-query'], capture_output=True,
                                     check=True, env={'DISPLAY': ':0'}, timeout=2)
    assert response.status_code == 200
...
def test_set_keyboard_layout_success(app, mocker):
    ...
    run_mock.assert_called_once_with(['raspi-config', 'nonint', 'do_configure_keyboard', 'ad'],
                                     capture_output=False, check=True, env={'DISPLAY': ':0'}, timeout=30)
...
def test_set_keyboard_layout_failure_wrong_type(app):
    validation_error_response = app.post(
        '/set-keyboard-layout', json={'layout': 1})
...
def test_set_keyboard_layout_invalid_code(app, mocker, restore_files):
    ...
    run_mock.assert_called_once_with(['raspi-config', 'nonint', 'do_configure_keyboard', 'fake-layout'],
                                     capture_output=False, check=True, env={'DISPLAY': ':0'}, timeout=30)
