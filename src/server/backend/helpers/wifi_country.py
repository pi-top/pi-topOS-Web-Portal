...
def list_wifi_countries() -> dict:
    ...
    with open(iso_countries()) as file:
        country_pairs = [line.rstrip().split()
...
def current_wifi_country() -> str:
    ...
    wifi_country = run_command(
        "raspi-config nonint get_wifi_country", check=False, timeout=5).strip()
...
def set_wifi_country(wifi_country_code):
    ...
    if code not in country_codes:
        PTLogger.error(
            "Unable to set Wi-Fi country - Not available: %s" % code)
        return None

    return run_command(
        "raspi-config nonint do_wifi_country %s" % code,
        timeout=5
    )
